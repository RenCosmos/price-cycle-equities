...
此流程仅用于日线 OHLCV 数据。它会以同一输入、同一参数计算指标、事件账本，以及
由六个来源阶段家族展开的八个方向化候选，再输出 JSON 与 Markdown。检测规则仍属于
实验性研究参数，不代表已经验证的交易优势。

v0.2.0-alpha.1 中，原命令内部通过 `manual_csv` provider 和统一 Router
加载，但命令参数不变。CSV 仍是唯一默认来源，不会自动联网。
